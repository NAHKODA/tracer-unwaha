<?php
//session_start();
//if(!empty($_SESSION['nama']) AND !empty($_SESSION['nim'])){
?>
	<header class="sb-page-header" style="padding-top: 60px;padding-bottom: 30px;text-align: left;font-size: 24px;padding: 30px 15px;color: rgba(255,255,255,.8);background-color: #26A65B;margin-bottom: 40px;font-size: 15px;background-image: url(images/bg.png);position: relative;">
	<div class="container" style="margin-top: 75px; margin-left:0px">
	    <div class="row">
	      <div class="col-md-4">
	        <ol class="breadcrumb" style="margin-left:90px">
	          <li><a href="/">Home</a></li>
	          <li class="active">Halaman 2A</li>
	        </ol>
	      </div>
	    </div>
	 </div> 
	</header>	
	<div class="container">		
		<div class="col-md-12">			
	    <div class="panel panel-success" style="box-shadow: 0 2px 5px 0 rgba(0, 0, 0, 0.16), 0 2px 10px 0 rgba(0, 0, 0, 0.12);">
	      <div class="panel-heading" style="background-image: url('data:image/svg+xml;base64,'); position: relative; background-color: rgb(38, 166, 91); color: rgb(255, 255, 255);position:relative;background-color:rgb(92,184,92);color:=rgb(255,255,255);">Halaman <b>2</b> dari 3 halaman</div>
	      <div class="panel-body">
	      	<div class="step-2B">
			<form action ="<?php print $config['base_url'] ?>simpan-step2A.php" method="POST" role="form">
			  
			  <div class="form-group">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Lama waktu menunggu sampai mendapatkan pekerjaaan yang pertama kali sesudah lulus sarjana<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="menunggukerja" value="0" onchange='menunggu_pekerjaan()' id="kerjaA" checked>kurang dari 1 bulan</label>

				<label class="radio-inline"><input type="radio" name="menunggukerja" value="1" onchange='menunggu_pekerjaan()' id="kerja1" >1 bulan</label>

				<label class="radio-inline"><input type="radio" name="menunggukerja" value="2" onchange='menunggu_pekerjaan()' id="kerja2" >2 bulan</label>

				<label class="radio-inline"><input type="radio" name="menunggukerja" value="3" onchange='menunggu_pekerjaan()' id="kerja3" >3 bulan</label>

				<label class="radio-inline"><input type="radio" name="menunggukerja" value="4" onchange='menunggu_pekerjaan()' id="kerja4" >4 bulan</label>

				<label class="radio-inline"><input type="radio" name="menunggukerja" value="5" onchange='menunggu_pekerjaan()' id="kerjaLainnya" >Lainnya... (bulan)</label>

				<br>
				<label style="margin-left:3px;margin-top:7px;font-weight:300">Dalam Bulan</label>
			  	<input type="number" name="menunggukerja2" value ="5" class="form-control" placeholder="Tuliskan alasannya. Hanya isi jika memilih Lainnya." autocomplete="off" style="margin-top:0px" disabled id="nunggukerja">
			  </div>		  

			  <div class="form-group">
			    <label for="namaperusahaan">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Nama perusahaan / instansi saudara bekerja pertama kali tersebut<span style="color:red">*</span>  </span>
			    </label>
			    <input type="text" name="namaperusahaan" class="form-control" id="hp" placeholder="Nama Perusahaan Saudara Bekerja">
			  </div>

			  <div class="form-group">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bentuk perusahaan<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="bentukperusahaan" value="Negeri" onchange='bentuk_perusahaan()' id="negeri" checked>Negeri</label>

				<label class="radio-inline"><input type="radio" name="bentukperusahaan" value="Swasta" onchange='bentuk_perusahaan()' id="swasta" >Swasta</label>

				<label class="radio-inline"><input type="radio" name="bentukperusahaan" value="BUMN" onchange='bentuk_perusahaan()' id="BUMN" >BUMN</label>

				<label class="radio-inline"><input type="radio" name="bentukperusahaan" onchange='bentuk_perusahaan()' value="0"id="bentuklainnya" >Lainnya...Sebutkan</label>

			  	<textarea type="text" name="bentukperusahaan2" rows="3" class="form-control" placeholder="Tuliskan Bentuk Perusahaan Lainnya" style="margin-top:5px" disabled id="bentuklain"></textarea>

			  </div>

			  <div style="border-style: dotted;border-color:#66AE53">
			  <p>
			  <div class="form-group" style="margin-left:10px">
			    <label for="bidangpekerjaan">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bidang Pekerjaan<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="bidangpekerjaan" value="TI" id="kerjaTI" onchange="ti()" checked="checked">TI</label>

				<label class="radio-inline"><input type="radio" name="bidangpekerjaan" value="Non TI" id="kerjanonTI" onchange="ti()">Non TI</label>
			  </div>
		  			  
			  <div class="form-group" id="TI" style="display:block;margin-left:10px">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bila Jawaban No. 4 Teknologi Informasi, jenis perusahaannya adalah<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="Kontraktor bidang TI" onchange='perusahaan_IT()' id="kontraktor" checked>Kontraktor bidang TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="Konsultan TI" onchange='perusahaan_IT()' id="konsultan" >Konsultan TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="Programmer " onchange='perusahaan_IT()' id="programmer" >Programmer </label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="Perusahaan bidang TI" onchange='perusahaan_IT()' id="perusahanbidanIT" >Perusahaan bidang TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="Instansi Pemerintahan" onchange='perusahaan_IT()' id="instansipemerintahan" >Instansi Pemerintahan</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="LSM 	bidang TI" onchange='perusahaan_IT()' id="lsmbidangit" >LSM bidang TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT" value="0" onchange='perusahaan_IT()' id="perusahaanIT" >Lainnya...Sebutkan</label>

			  	<textarea type="text" name="perusbidangIT2" rows="3" class="form-control" placeholder="Tuliskan alasannya. Hanya isi jika memilih Lainnya." autocomplete="off" style="margin-top:5px;width:99%" disabled id="ITperusahaan"></textarea>
			  </div>

			  <div class="form-group" id="nonTI" style="display:none;margin-left:10px">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bila jawaban No. 4  Non TI, jenis perusahaan <span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="perusbidangnonIT" value="BANK" onchange='perusahaan_nonIT()' id="bank" checked>BANK</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT" value="Perusahaan Manufaktur" onchange='perusahaan_nonIT()' id="perusmanufaktur" >Perusahaan Manufaktur</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT" value="Konsultan Keuangan" onchange='perusahaan_nonIT()' id="konsultankeuangan" >Konsultan Keuangan </label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT" value="Kontraktor non TI" onchange='perusahaan_nonIT()' id="kontraktornonit" >Kontraktor non TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT" value="LSM" onchange='perusahaan_nonIT()' id="lsm" >LSM</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT" value="0" onchange='perusahaan_nonIT()' id="perusahaannonIT" >Lainnya...Sebutkan</label>

			  	<textarea type="text" name="perusbidangnonIT2" rows="3" class="form-control" placeholder="Tuliskan alasannya. Hanya isi jika memilih Lainnya." autocomplete="off" style="margin-top:5px;width:99%" disabled id="ITnonperusahaan"></textarea>
			  </div>	
			  <p>	
			  </div>	  
			  <p>

			  <div class="form-group">
			    <label for="namaperusahaan">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Jabatan saat pertama diterima bekerja<span style="color:red">*</span>  </span>
			    </label>
			    <input type="text" name="jabatankerja" class="form-control" id="jabatankerja" placeholder="Contoh : Pegawai" autocomplete="off">
			  </div>

			  <div class="form-group">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Perkiraan income / pendapatan pertama yang diterima perbulan (bukan hanya gaji, tapi total income<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="income" value="500 ribu – 750 ribu" checked>500 ribu – 750 ribu</label>

				<label class="radio-inline"><input type="radio" name="income" value="750 ribu – 1 juta">750 ribu – 1 juta</label>

				<label class="radio-inline"><input type="radio" name="income" value="1 juta – 1,5 juta">1 juta – 1,5 juta</label>

				<label class="radio-inline"><input type="radio" name="income" value="1,5 juta – 2 juta">1,5 juta – 2 juta</label>

				<label class="radio-inline"><input type="radio" name="income" value="2 juta – 3 juta">2 juta – 3 juta</label>

				<label class="radio-inline"><input type="radio" name="income" value="> 3 juta"> > 3 juta</label>

			  </div>

			  <div class="form-group">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; <b>Saat ini</b>, anda bekerja di<span style="color:red">*</span> </span>
			    </label><br>

			    <label style="margin-left:20px;margin-bottom:5px" class="radio-inline"><input type="radio" onchange="bekerja_sekarang()" name="bekerjasekarang" id="tetap"> Tetap, di perusahaan pertama setelah lulus sarjana</label><br>

			    <div class="bekerjasekarang" style="margin-left:50px;display:none;border-style: dotted;border-color:#66AE53" id="jabsekarang">
			    <label style="margin-top:5px;margin-left:5px">Jabatan saat ini sebagai</label>

			    <input style="margin-left:5px;width:98%;" type="text" name="jabatankerja" class="form-control" id="jabatan" placeholder="Jabatan Saat Pertama Bekerja" autocomplete="off" >

			    <label style="margin-top:5px;margin-left:5px">Perkiraan income / pendapatan</label>
			    <br>

			    <label style="margin-left:5px;margin-top:0px;margin-bottom:5px" class="radio-inline"><input type="radio" name="perkiraanincome"  id="tetapno8"> Tetap seperti No. 8</label>

			    <label style="margin-left:20px;margin-bottom:5px" class="radio-inline"><input type="radio" name="perkiraanincome"  id="meningkat"> Sudah meningkat</label><br>
			    </div>

			    <label style="margin-left:20px;margin-top:10px" class="radio-inline"><input type="radio" name="bekerjasekarang" onchange="bekerja_sekarang()" id="belumkerja"> Belum bekerja, atau sedang dalam masa transisi / pindah pekerjaan</label><br>	

			    <label style="margin-left:20px" class="radio-inline"><input type="radio" name="bekerjasekarang" onchange="bekerja_sekarang()" id="berlainan">Berlainan dengan perusahaan pertama setelah lulus sarjana</label>	    

			  </div>

			  <div class="form-group">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Sampai saat ini, telah berpindah tempat bekerja sebanyak<span style="color:red">*</span> </span>
			    </label><br>

				<label class="radio-inline"><input type="radio" name="pindahkerja" value="2 Kali" checked>2 Kali</label>

				<label class="radio-inline"><input type="radio" name="pindahkerja" value="3 Kali">3 Kali</label>

				<label class="radio-inline"><input type="radio" name="pindahkerja" value="4 Kali">4 Kali</label>

				<label class="radio-inline"><input type="radio" onchange='pindah_kerja()' name="pindahkerja" id="pindahkerja">Diatas 4 Kali...Sebutkan</label>

				<textarea type="text" name="pindahkerja" rows="3" class="form-control" placeholder="Tuliskan alasannya. Hanya isi jika memilih Diatas 4 Kali." autocomplete="off" style="margin-top:5px" disabled id="kerjapindah"></textarea>
			  </div>

			  <div class="perusahaansekarang" style="display:none;border-style: dotted;border-color:#66AE53;margin-bottom:0px;margin-top:10px" id="perussekarang">
			  <div class="form-group" style="margin-top:10px">
			    <label for="namaperusahaansekarang">
			    	<span style="border-left: 3px solid #66AE53;margin-left:10px;"> &nbsp; Nama perusahaan tempat bekerja sekarang<span style="color:red">*</span>  </span>
			    </label>
			    <input type="text" style=";margin-left:10px;width:98%" name="perusahaansekarang" class="form-control" id="perusahaansekarang" placeholder="Nama perusahaan tempat bekerja sekarang" autocomplete="off">
			  </div>

			  <div class="form-group">
			    <label for="jabatanperusahaansekarang">
			    	<span style="border-left: 3px solid #66AE53;margin-left:10px"> &nbsp; Jabatan di perusahan anda bekerja sekarang<span style="color:red">*</span>  </span>
			    </label>
			    <input type="text" style=";margin-left:10px;width:98%" name="jabatanperusahaansekarang" class="form-control" id="jabatanperusahaansekarang" placeholder="Jabatan di perusahan anda bekerja sekarang" autocomplete="off">
			  </div>			  			  

			  <div class="form-group">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;margin-left:10px"> &nbsp; Bentuk perusahaan<span style="color:red">*</span> </span>
			    </label><br>
			    <div style="margin-left:10px";
				<label class="radio-inline"><input type="radio" name="bentukperusahaan2" value="Negeri" onchange='bentuk_perusahaan2()' id="negeri2" checked>Negeri</label>

				<label class="radio-inline"><input type="radio" name="bentukperusahaan2" value="Swasta" onchange='bentuk_perusahaan2()' id="swasta2" >Swasta</label>

				<label class="radio-inline"><input type="radio" name="bentukperusahaan2" value="BUMN" onchange='bentuk_perusahaan2()' id="BUMN2" >BUMN</label>

				<label class="radio-inline"><input type="radio" name="bentukperusahaan2" onchange='bentuk_perusahaan2()' id="bentuklainnya2" >Lainnya...Sebutkan</label>
				</div>
			  	<textarea type="text" name="bentukperusahaan2" rows="3" class="form-control" placeholder="Tuliskan Bentuk Perusahaan Lainnya" autocomplete="off" style="margin-top:5px;margin-left:10px;width:98%" disabled id="bentuklain2"></textarea>

			  </div>

			  <div style="">
			  <p>
			  <div class="form-group" style="margin-left:10px">
			    <label for="bidangpekerjaan">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bidang Pekerjaan<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="bidangpekerjaan2" value="TI" id="kerjaTI2" onchange="ti2()" checked="checked">TI</label>

				<label class="radio-inline"><input type="radio" name="bidangpekerjaan2" value="Non TI" id="kerjanonTI2" onchange="ti2()">Non TI</label>
			  </div>
		  			  
			  <div class="form-group" id="TI2" style="display:block;margin-left:10px">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bila Jawaban No. 4 Teknologi Informasi, jenis perusahaannya adalah<span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="Kontraktor bidang TI" onchange='perusahaan_IT2()' id="kontraktor2" checked>Kontraktor bidang TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="Konsultan TI" onchange='perusahaan_IT2()' id="konsultan2" >Konsultan TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="Programmer " onchange='perusahaan_IT()' id="programmer2" >Programmer </label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="Perusahaan bidang TI" onchange='perusahaan_IT2()' id="perusahanbidanIT2" >Perusahaan bidang TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="Instansi Pemerintahan" onchange='perusahaan_IT2()' id="instansipemerintahan2" >Instansi Pemerintahan</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="LSM 	bidang TI" onchange='perusahaan_IT2()' id="lsmbidangit2" >LSM bidang TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangIT2" value="2 bulan" onchange='perusahaan_IT2()' id="perusahaanIT2" >Lainnya...Sebutkan</label>

			  	<textarea type="text" name="perusbidangIT2" rows="3" class="form-control" placeholder="Tuliskan alasannya. Hanya isi jika memilih Lainnya." autocomplete="off" style="margin-top:5px;width:99%" disabled id="ITperusahaan2"></textarea>
			  </div>

			  <div class="form-group" id="nonTI2" style="display:none;margin-left:10px">
			    <label for="jenisusaha">
			    	<span style="border-left: 3px solid #66AE53;"> &nbsp; Bila jawaban No. 4  Non TI, jenis perusahaan <span style="color:red">*</span> </span>
			    </label><br>
				<label class="radio-inline"><input type="radio" name="perusbidangnonIT2" value="BANK" onchange='perusahaan_nonIT2()' id="bank2" checked>BANK</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT2" value="Perusahaan Manufaktur" onchange='perusahaan_nonIT2()' id="perusmanufaktur2" >Perusahaan Manufaktur</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT2" value="Konsultan Keuangan" onchange='perusahaan_nonIT2()' id="konsultankeuangan2" >Konsultan Keuangan </label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT2" value="Kontraktor non TI" onchange='perusahaan_nonIT2()' id="kontraktornonit2" >Kontraktor non TI</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT2" value="LSM" onchange='perusahaan_nonIT2()' id="lsm2" >LSM</label>

				<label class="radio-inline"><input type="radio" name="perusbidangnonIT2" value="2 bulan" onchange='perusahaan_nonIT2()' id="perusahaannonIT2" >Lainnya...Sebutkan</label>

			  	<textarea type="text" name="perusbidangnonIT2" rows="3" class="form-control" placeholder="Tuliskan alasannya. Hanya isi jika memilih Lainnya." autocomplete="off" style="margin-top:5px;width:99%" disabled id="ITnonperusahaan2"></textarea>
			  </div>	
			  <p>	
			  </div>	
			  </div>
			  <p>
			  <br>
			  <button type="submit" class="btn btn-default" style="float:right">Next Step <i class="fa fa-long-arrow-right"></i></button>
			</form>	
			Note : <span style="color:red">*</span>) Wajib Diisi  
			</div>
	      </div>
	    </div>						
		</div>
	</div>
<?php
/*
}else{
       
       header("location:home");
}
*/
?>